...
var Combobox = React.createClass({
  ...
  */
  autocompleteInputValue: function() {
    ...
    var allowExactMatch = (!this.state.selectionRange && this.props.children.length === 1);
    var fragment = matchFragment(inputValue, label, allowExactMatch);
...
function matchFragment(userInput, firstChildLabel, allowMatch) {
  userInput = userInput.toLowerCase();
  firstChildLabel = firstChildLabel.toLowerCase();
  var exactMatch = (userInput === firstChildLabel);
  if (userInput === '' || (exactMatch && !allowMatch))
    return false;
